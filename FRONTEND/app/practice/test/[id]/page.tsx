...
import Link from "next/link"
import {
  Award, Clock, ArrowLeft, ArrowRight, Flag, CheckCircle, XCircle, RotateCcw
} from "lucide-react"
import {
...
export default function PracticeTestPage() {
  ...
  const [testData, setTestData] = useState<PracticeTest | null>(null)
  const [currentQuestion, setCurrentQuestion] = useState(0)
  const [answers, setAnswers] = useState<Record<number, string>>({})
  ...
  useEffect(() => {
    const fetchTest = async () => {
      const res = await fetch(`http://localhost:5000/api/practice-tests/${id}`)
  ...
  if (!testData) return <p className="text-center py-12">Loading test...</p>

  // helpers
  const formatTime = (sec: number) => `${Math.floor(sec / 60)}:${(sec % 60).toString().padStart(2, "0")}`
  const handleAnswer = (qid: number, ans: string) => setAnswers({ ...answers, [qid]: ans })
  const handleSubmit = () => setIsCompleted(true)
  const score = testData.questions.filter((q) => answers[q.id] === q.correctAnswer).length
  ...
  if (!isStarted) {
    return (
      <div className="max-w-4xl mx-auto py-12">
        <Card>
          <CardHeader>
            <Badge>{testData.subject}</Badge>
            <CardTitle>{testData.title}</CardTitle>
            <CardDescription>{testData.description}</CardDescription>
          </CardHeader>
          <CardContent>
            <p>{testData.questions.length} Questions · {testData.timeLimit} min · {testData.difficulty}</p>
            <Button onClick={() => setIsStarted(true)} className="mt-6">Start Test</Button>
          </CardContent>
        </Card>
      </div>
    )
      ...
      <div className="max-w-4xl mx-auto py-12">
        <Card>
          <CardHeader>
            ...
            <CardDescription>{score} of {testData.questions.length} correct</CardDescription>
          </CardHeader>
          <CardContent>
            <Button asChild><Link href="/practice">Try Another</Link></Button>
            <AchievementShare achievement={{ title: "Completed a Test", description: "Practice Finished", category: "completion" }} score={percentage} testName={testData.title} />
            <SocialShare title={`I scored ${percentage}%`} description={`on ${testData.title}`} hashtags={["11Plus", "ExamPrep"]} />
          </CardContent>
        </Card>
      </div>
    )
  ...
  return (
    <div className="max-w-4xl mx-auto py-12">
      <div className="flex justify-between mb-4">
        <span>{`Question ${currentQuestion + 1}/${testData.questions.length}`}</span>
      ...
      <Card className="mt-4">
        ...
        <CardContent>
          ...
          <div className="flex justify-between mt-6">
            <Button disabled={currentQuestion === 0} onClick={() => setCurrentQuestion((p) => p - 1)}>Previous</Button>
